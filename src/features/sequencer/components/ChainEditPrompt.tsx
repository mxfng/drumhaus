...
export const ChainEditPrompt: React.FC = () => {
  ...
  return (
    <Tooltip
      content="Click A/B/C/D to build a chain sequence (up to 8 bars total). Click 'vari chain' again to save."
      side="top"
    >
      <div className="bg-instrument flex h-full flex-col gap-1 pt-1">
        <div className="flex flex-1 flex-wrap items-center gap-1 px-1 lowercase">
          {chain.steps.map((step, idx) => {
            ...
            return (
              <React.Fragment key={`${step.variation}-${idx}`}>
                <div
                  className={cn(
                    "flex items-center justify-center gap-1 rounded-tl-md rounded-br-md border px-1 text-xs",
                    isActive
          ...
          {totalBars < MAX_CHAIN_STEPS && (
            <>
              ...
              <div className="border-foreground-muted text-foreground flex items-center justify-center rounded-tl-md rounded-br-md border border-dashed text-xs">
                <Plus size={16} />
              </div>
            </>
          )}
